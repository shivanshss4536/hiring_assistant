...
user_input = st.text_input("💬 Type your message here...", key="user_input", disabled=not st.session_state.active)
col1, col2 = st.columns([1, 4])
with col1:
    if st.button("🚀 Send", disabled=not st.session_state.active or not user_input.strip()):
        st.session_state.chat_history.append(("user", user_input))
        bot_response, ended = st.session_state.assistant.process_message(user_input)
